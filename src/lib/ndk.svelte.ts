  ...
  try {
    ...
    if (window.nostr) {
      try {
        ...
        console.warn(
          'Failed to initialize NIP-07 signer, falling back to private key signer.',
          e
        );
      }
    }
    ...
    if (storedSigner) {
      return setSigner(
        await NDKPrivateKeySigner.fromPayload(storedSigner, ndk)
      );
    }
    ...
    if (ncryptsec) {
      return setSigner(
        NDKPrivateKeySigner.fromNcryptsec(ncryptsec, 'password', ndk)
      );
    }
