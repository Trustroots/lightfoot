<script lang="ts">
  ...
  function getUsername(entry: SingleProperties): string | undefined {
    ...
    if (
      ...
      typeof (entry.user as { name?: unknown }).name === 'string'
    ) {
      return (entry.user as { name: string }).name;
    }
...
{#snippet note(entry: SingleProperties)}
  ...
  <div class="space-y-3 border-b border-gray-300 last-of-type:border-0">
    {#if !entry.verified}
      <div
        class="flex items-center gap-2 text-[10px] uppercase tracking-wide text-red-500"
      >
        <span
          class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 border border-red-500"
        >
    ...
    <details class="space-y-4">
                ...
                {:then profile}
                  {#if profile}
                    <button
                      ...
                      onclick={() =>
                        setSelectedUser({ ...profile, pubkey: entry.pubkey })}
                    >
      ...
      <pre
        class="bg-gray-100 dark:bg-gray-800 p-2 text-xs whitespace-pre-wrap">{JSON.stringify(
...
{#if selectedFeature.cluster}
  ...
  {:then children}
    <details class="mb-4">
      <summary class="flex list-none items-center justify-between">
        <h2 class="font-bold">
          Cluster ({selectedFeature.point_count} points)
        </h2>
        <span class="cursor-pointer text-xs text-gray-400">Show raw data</span>
      </summary>
      <pre
