...
export abstract class IEventProcessor {
  ...
  protected extractLocation(
    ...
    const longestGTag = event.tags
      .filter(
        (tag: any) =>
          tag[0] === 'g' && typeof tag[1] === 'string' && tag[1].length
      )
      .reduce(
        (longest: string, tag: any) =>
          tag[1].length > longest.length ? tag[1] : longest,
        ''
      );
    ...
    const latLngMatch = longestGTag.match(
      /^(-?\d+(\.\d+)?),\s*(-?\d+(\.\d+)?)$/
    );
    if (latLngMatch) {
    ...
    return {
      lngLat: [location.longitude[2], location.latitude[2]],
      geohash: longestGTag,
    };
  }
}
