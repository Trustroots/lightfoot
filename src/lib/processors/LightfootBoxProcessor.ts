...
export class LightfootBoxProcessor extends IEventProcessor {
	async process(event: any): Promise<GeoJSONFeature<Geometry, SingleProperties> | null> {
		...
		const hasLightfootBoxTag = event.tags?.some((tag: any) => tag[0] === 't' && tag[1] === '#lightfootbox');
		...
		try {
			...
			if (parsed.availability && ['public', 'scheduled', 'host', 'transient'].includes(parsed.availability)) {
